...
SPOOL &1\BENEFICIARIO\BENEFICIARIOS_UNICOO_NAO_MIGRADOS.html

--ATIVOS
select count(*) QT_BENEF_ATIVOS_NAO_MIGRADOS
 ...
 where not exists (select 1
   ...
   and not exists(select 1 from temp_plano_nao_migrar tp where tp.cdcontrato = us.cdcontrato)
   and not exists(select 1 from temp_contrato_nao_migrar tc where tc.nrcontrato = us.nrcontrato)
and (us.dtexclusao is null or us.dtexclusao > sysdate);

--LISTAR BENEFICIARIOS DO UNICOO QUE NAO FORAM MIGRADOS PARA O TOTVS
select us.nrsequencial_usuario, us.nrregistro, us.nrcontrato, us.nrfamilia, us.nrregistro_usuario, us.tpusuario, us.dtinicio, us.dtexclusao
  from producao.usuario@unicoo_homologa us
 where not exists (select 1
          from gp.import_bnfciar ib
         where ib.num_seqcial_bnfciar = us.nrsequencial_usuario)
