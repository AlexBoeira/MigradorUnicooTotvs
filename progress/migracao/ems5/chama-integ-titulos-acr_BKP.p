...
REPEAT:
   ...
   */
  select count(*) into nr-pendentes-na-fila-aux from ti_tit_acr where ti_tit_acr.cdsituacao = ENTRY(1, in-param-aux).
  if nr-pendentes-na-fila-aux = 0
  then leave.

  PROCESS EVENTS.
  RUN ems5/ti_tit_acr.p (INPUT ENTRY(1, in-param-aux)).
