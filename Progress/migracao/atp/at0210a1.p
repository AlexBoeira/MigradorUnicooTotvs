...
then do:
       run escrever-log("Parametros do sistema nao cadastrados.").
       return.
     end.
...
then do:
       run escrever-log("Parametros do modulo Revisao de Contas nao cadastrados.").
       return.
     end.
...
THEN DO:
       run importa-dados.
       run escrever-log("@@@@@saindo de at0210a1.p").
     END.
ELSE DO:
...
PROCEDURE importa-dados:
    DEFINE VARIABLE ix-aux AS INTEGER NO-UNDO.
	def var nr-guia-aux as int no-undo.
    ...
    run escrever-log("@@@@@Antes do loop principal (importa-dados)").
    ...
    for each import-guia where import-guia.ind-sit-import    = in-status-monitorar-par /* Recebido */
        ...
        /*ix-aux = ix-aux + 1.
        IF ix-aux > 1000 THEN return.*/

        run escrever-log("Importando Guia Princ.: " + string(import-guia.num-seqcial-control) + " Sit: " + in-status-monitorar-par).
    ...
    for each import-guia where import-guia.ind-sit-import    = in-status-monitorar-par /* Recebido */
        ...
        run escrever-log("Importando Guia Assoc.: " + string(import-guia.num-seqcial-control) + " Sit: " + in-status-monitorar-par).
    ...
    run escrever-log("@@@@@Apos o loop principal").
    
/*    run escrever-log("Antes de resetar SEQ-GUIAUTOR").

    /* resetar seq-guiautor ao final do processo */
    nr-guia-aux = 1.
    for last guiautor fields (nr-guia-atendimento) use-index guiauto1
                where guiautor.cd-unidade          = paramecp.cd-unimed
                  and guiautor.aa-guia-atendimento = year(today) no-lock:
        nr-guia-aux = guiautor.nr-guia-atendimento. 
    end.

    &if   "{&tipo-banco}" = "normal"
    &then assign current-value(seq-guiautor) = nr-guia-aux.
    &else do:
	 	   do transaction:
 			  ...
 			  run stored-proc shsrcadger.send-sql-statement hand1 = proc-handle
			  ("create sequence seq_guiautor start with " + string(nr-guia-aux) + " minvalue 0 maxvalue 99999999 nocycle nocache increment by 1").
    ...
    run escrever-log("@@@@@Apos resetar SEQ-GUIAUTOR. Novo valor: " + string(nr-guia-aux)).
*/
...
procedure cria-guia:
    ...
    assign guiautor.cd-userid-auditor        = ""                                                     
           ...
           guiautor.char-15                  = import-guia.cod-livre-1 /* HORA DA SOLICITACAO */
...
procedure valida-odontologia:
    ...
    THEN DO:
           run grava-erro(input import-guia.num-seqcial-control,
                          input "import-guia-odonto",
                          input "(1)Prestador executante nao encontrado.",
                          input today,
                          input " Unidade(import-guia-odonto.cd-unidade-exec): " + string(import-guia-odonto.cd-unidade-exec) + 
...
procedure valida-opme:
    ...
    then do: 
           ...
           run grava-erro(input import-guia.num-seqcial-control,
                          ...
                          input today,
                          input "import-anexo-opme.num-seqcial-import-anexo: " + char-aux1 + 
                                " import-anexo-solicit.num-seqcial: " + char-aux2).
...
procedure valida-prorrogacao:
    ...
    then do:
           run grava-erro(input import-guia.num-seqcial-control,
                          input "import-guia-prorrog",
                          input "Campo Indicacao clinica da guia de prorrogacao nao informado.",
                          input today,
                          input "import-guia-prorrog.num-seq-import: " + string(import-guia-prorrog.num-seq-import)).
...
procedure valida-internacao:
    ...
    then do:
           run grava-erro(input import-guia.num-seqcial-control,
                          ...
                          input today,
                          input "import-guia-intrcao.num-seq-import: " + string(import-guia-intrcao.num-seq-import)).
    ...
    then do:
           ...
           then do:
                  run grava-erro(input import-guia.num-seqcial-control,
                                 input "import-guia-intrcao",
                                 input "Campo Indicacao clinica da guia de internacao nao informado.",
                                 input today,
                                 input "import-guia-intrcao.num-seq-import: " + string(import-guia-intrcao.num-seq-import)).
    ...
    THEN DO:
           run grava-erro(input import-guia.num-seqcial-control,
                          input "import-guia-intrcao",
                          input "(2)Prestador executante nao encontrado.",
                          input today,
                          input " Unidade(import-guia-intrcao.cd-unidade-exec): " + string(import-guia-intrcao.cd-unidade-exec) + 
    ...
    ELSE DO:
            ...
            then do:
                   run grava-erro(input import-guia.num-seqcial-control,
                                  ...
                                  input "Vinculo nao encontrado para o prestador executante e especialidade informados.",
                                  input today,
                                  input " Unid: " + string(import-guia-intrcao.cd-unidade-exec) + 
...
procedure valida-sp-sadt:
    ...
    THEN DO:
           run grava-erro(input import-guia.num-seqcial-control,
                          input "import-guia-sadt",
                          input "(3)Prestador executante nao encontrado.",
                          input today,
                          input " Unidade(import-guia-sadt.cd-unidade-exec): " + string(import-guia-sadt.cd-unidade-exec) + 
    ...
    ELSE DO:
                ...
                then do:
                       run grava-erro(input import-guia.num-seqcial-control,
                                      ...
                                      input "Vinculo nao encontrado para o prestador executante e especialidade informados.",
                                      input today,
                                      input " Unid: "  + string(import-guia-sadt.cd-unidade-exec) + 
...
procedure valida-consulta:
    ...
    THEN DO:
           run grava-erro(input import-guia.num-seqcial-control,
                          input "import-guia-con",
                          input "(4)Prestador executante nao encontrado.",
                          input today,
                          input " Unidade(import-guia-con.cd-unidade-exec): " + string(import-guia-con.cd-unidade-exec) + 
    ...
    ELSE DO:
            ...
            then do:
                   run grava-erro(input import-guia.num-seqcial-control,
                                  ...
                                  input "Vinculo nao encontrado para o prestador executante e especialidade informados.",
                                  input today,
                                  input " Unid: "  + string(import-guia-con.cd-unidade-exec) + 
                                        " Prest: " + string(import-guia-con.cd-prestador-exec) +  
    ...
    then do:
           ...
           then do:
                  run grava-erro(input import-guia.num-seqcial-control,
                                 ...
                                 input today,
                                 input "Carteira:" + STRING(import-guia.cd-carteira-usuario)).
     ...
     then do:
            run grava-erro(input import-guia.num-seqcial-control,
                                 input "import-guia",
                                 input "Carteira do beneficiario nao encontrada.",
                                 input today,
                                 input "Unidade/Carteira informada: "  + STRING(import-guia.cd-unidade-carteira) + STRING(import-guia.cd-carteira-usuario)).
...
procedure valida-dados-movimento:
    ...
    then do:
           run grava-erro(input import-guia.num-seqcial-control,
                          ...
                          input "Modulo de cobertura nao informado. (zero ou nulo)",
                          input today,
                          input "Tp.Guia(import-guia-movto.ind-tip-guia): "      + string(import-guia-movto.ind-tip-guia) + 
                                " Nro.Seq.Guia(import-guia-movto.val-seq-guia): " + string(import-guia-movto.val-seq-guia)).
           ...
           then do:
                  ...
                  then do:
                         ...
                         then do:
                                ...
                                ELSE char-aux4 = "nulo".
                                IF import-guia-movto.cod-movto-guia <> ?
                                THEN char-aux5 = import-guia-movto.cod-movto-guia.
                                ELSE char-aux5 = "nulo".
                                run grava-erro(input import-guia.num-seqcial-control,
                                               input "import-guia-movto",
