...
rem ***********************************************************************************************************************************************************
set PROGRAMA=cgp\cg0410f_batch.p
set CLIENTLOG_NOME=c:\temp\clientlog_faturas_
...
if %FILA% leq %QT_FILAS_IMPORTACAO_FATURAS%  (
  call %PROWIN32% -basekey "ini" -ininame %INI% -pf %PF% -p %PROGRAMA% -param %FILA%,%EVENTO_FATURAS_MIGRADAS%,%EXECUTAR_CONTINUAMENTE% %CLIENTLOG_CONF% %CLIENTLOG_NOME%%FILA%_%DATA_HORA%
