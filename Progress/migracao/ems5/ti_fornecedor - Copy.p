...
for each TI_FORNECEDOR EXCLUSIVE-LOCK where TI_FORNECEDOR.cdsituacao = in-status-par:
    ...
    THEN DO:
           run p_grava_falha("TI_FORNECEDOR " + string(ti_FORNECEDOR.COD_FORNECEDOR) + " esta no grupo NAOM (nao migrar) mas possui titulos em aberto. Verifique","",11).
         END.
    ELSE DO:
           ...
           THEN do:                                           
                  run p_grava_falha('N∆o existe de-para na tabela TI_PESSOA para o pessoa ' + string(ti_FORNECEDOR.nrpessoa),"",11).
                END.  
           else do:
                  /**
                  ...
                  THEN DO:
                         run p_grava_falha('Codigo do fornecedor nao esta informado. Essa informacao deve ser gerada no processo anterior (extrator SQL). Pessoa: ' + string(ti_FORNECEDOR.nrpessoa),"",11).
                       END.
                  ELSE DO:
                         ...
                         then do:
                                ...
                                THEN DO:
                                       run p_grava_falha('Codigo do cliente (' + STRING(ti_pessoa.cdn_cliente) + ') esta diferente do codigo do fornecedor (' + string(ti_pessoa.cdn_fornecedor) + 
                                                         '). Revise a carga desses campos, pois ambos devem ter a mesma codificacao. Pessoa: ' + string(ti_FORNECEDOR.nrpessoa),"",11).
                         ...
                         then run p_grava_falha('Codigo do CNPJ e CEI do fornecedor (' + STRING(ti_pessoa.cdn_fornecedor) + ') zerado.',"",11).
...
/*
   ...
   Este atributo ser† utilizado para que a API receba o tipo de operaá∆o a ser efetuado. 
   Quando informado 1, ser† efetuada a inclus∆o ou, caso o registro j† estiver cadastrado, a modificaá∆o. 
   E, quando for informado 2, ser† efetuada a eliminaá∆o.

   ASSIGN TI_FORNECEDOR.cod_acao = "E" .
*/
   ASSIGN ttv_num_tip_operac-aux = 1.

   /*if TI_FORNECEDOR.cod_acao = "I" 
   or TI_FORNECEDOR.cod_acao = "A"
   then assign ttv_num_tip_operac-aux = 1.
   */
   ...
   if ind_tipo_pessoa_par='F' then do:
        ...
        if not available pessoa_fisic  then do:
          ...
          run p_grava_falha(
            ...
            /*============================================*/
            /* Verifica se alteraá∆o de endereáo Ç valida */
            /*--------------------------------------------*/
            if ti_fornecedor.nom_bairro=' ' and pessoa_fisic.nom_bairro<>' ' then do:
                 run p_grava_falha(
                                        'O fornecedor esta cadastrado com o bairro << ' + pessoa_fisic.nom_bairro + ', n∆o Ç permitido alterar o bairro para espaáo em branco',
                                        'A Pessoa ' + string(num_pessoa_par)+ ' do fornecedor '+ string(cod_fornecedor_par) + 'n∆o existe no ',
                                        21
                                        ).           
            ...
            if not available pessoa_jurid  then do:
               ...
               run p_grava_falha(
                   ...
                   if ti_fornecedor.nom_bairro=' ' and pessoa_jurid.nom_bairro<>' ' then do:
                        ...
                        run p_grava_falha(
                                               'O fornecedor esta cadastrado com o bairro << ' + pessoa_jurid.nom_bairro + ', n∆o Ç permitido alterar o bairro para espaáo em branco',
                                               'A Pessoa ' + string(num_pessoa_par)+ ' do fornecedor '+ string(cod_fornecedor_par) + 'n∆o existe no ',
                                               21
                                               ).           
          ...
          then do:
                 ...
                 then do:  
                        ...
                        then do:
                               ...
                               assign
                               TI_FALHA_DE_PROCESSO.CDINTEGRACAO = "TI_FORNECEDOR"
                               TI_FALHA_DE_PROCESSO.TXAJUDA = 'N∆o Ç poss°vel excluir o FORNECEDOR, pois, o codigo de FORNECEDOR informado n∆o foi encontratdo'
                               ...
                               assign
                               TI_FALHA_DE_PROCESSO.CDINTEGRACAO = "TI_FORNECEDOR"
                               TI_FALHA_DE_PROCESSO.TXAJUDA = 'N∆o Ç poss°vel alterar o FORNECEDOR, pois, o codigo de FORNECEDOR informado n∆o foi encontratdo'
                               TI_FALHA_DE_PROCESSO.TXFALHA = 'FORNECEDOR : ' + string(ti_FORNECEDOR.cod_FORNECEDOR) + ' n∆o localizado'
                               TI_FALHA_DE_PROCESSO.NRMENSAGEM = 11         
                               TI_FALHA_DE_PROCESSO.NRSEQ_CONTROLE_INTEGRACAO = ti_FORNECEDOR.nrseq_controle_integracao.                                                                                                         
                             END.
