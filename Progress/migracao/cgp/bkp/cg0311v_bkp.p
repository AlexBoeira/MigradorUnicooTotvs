...
PROCEDURE cria-beneficiario:
    ...
    do on error undo, retry:
       ...
       FOR FIRST propost USE-INDEX propo24
           WHERE propost.nr-contrato-antigo = b-import-bnfciar.nr-contrato-antigo EXCLUSIVE-LOCK: /* &&&&&retirar regra de afericao e transformar em NO-LOCK*/
       END.
       ...
       FOR FIRST b-usuario FIELDS (cd-titular id-pessoa cd-usuario) USE-INDEX usuari26
       ...
       then do:
              ...
              if b-import-bnfciar.cd-registro-plano <> 0 
              then for first reg-plano-saude fields(cdn-plano-ans idi-registro)
        ...
        */
       RUN escrever-log("@@@@@Antes de buscar proximo ct-usuario").
       ...
       RUN escrever-log("@@@@@Antes de criar USUARIO").
       ...
       REPEAT:
           ...
           THEN do:
                  ...
                  ASSIGN ct-codigo = ct-codigo + 1.

                  RUN escrever-log("@@@@@ct-usuario ja esta em uso. Vai gerar novo: " + STRING(ct-codigo)).
                END.
           else do:
       ...
       ASSIGN usuario.id-pessoa                = b-import-bnfciar.num-seqcial
              usuario.nm-usuario               = caps(tt-import-bnfciar.nome-usuario)
       ...
       THEN DO:
              ASSIGN usuario.nr-identidade            = IF (pessoa-fisica.nr-identidade <> "" or pessoa-fisica.nr-identidade <> ?)
                                                        THEN pessoa-fisica.nr-identidade
                                                        ELSE b-import-bnfciar.nr-identidade
                     usuario.uf-emissor-doc           = IF (pessoa-fisica.uf-emissor-ident <> "" or pessoa-fisica.uf-emissor-ident <> ?)
                                                        THEN CAPS(pessoa-fisica.uf-emissor-ident)
                                                        ELSE CAPS(b-import-bnfciar.uf-emissor-ident)
                     usuario.ds-orgao-emissor-ident   = IF (pessoa-fisica.ds-orgao-emissor-ident <> "" or pessoa-fisica.ds-orgao-emissor-ident <> ?)
                     ...
                     usuario.nm-pais                  = IF (pessoa-fisica.nm-pais-emissor-ident <> "" or pessoa-fisica.nm-pais-emissor-ident <> ?)
                                                        THEN CAPS(pessoa-fisica.nm-pais-emissor-ident)
                                                        ELSE CAPS(b-import-bnfciar.nom-pais)
                     usuario.cd-cpf                   = IF (pessoa-fisica.cd-cpf <> "" or pessoa-fisica.cd-cpf <> ?)
                     ...
                     usuario.nm-mae                   = IF (pessoa-fisica.nm-mae <> "" or pessoa-fisica.nm-mae <> ?)
                                                        THEN CAPS(pessoa-fisica.nm-mae)
                                                        ELSE CAPS(b-import-bnfciar.nom-mae)
                     usuario.nm-pai                   = IF (pessoa-fisica.nm-pai <> "" or pessoa-fisica.nm-pai <> ?)
                     ...
                     usuario.char-29                  = pessoa-fisica.char-2.

              if b-import-bnfciar.log-respons  = NO
              AND AVAIL b-usuario /* titular */
              THEN DO:
                     FIND FIRST pessoa-fisica WHERE pessoa-fisica.id-pessoa = b-usuario.id-pessoa NO-LOCK NO-ERROR.
       ...
       THEN DO:
              ...
              FIND FIRST gerac-cod-ident-propost EXCLUSIVE-LOCK
                              WHERE gerac-cod-ident-propost.cdn-modalid = b-import-bnfciar.cd-modalidade
              ...
              THEN DO:
                     ...
                     assign gerac-cod-ident-propost.cdn-modalid         = b-import-bnfciar.cd-modalidade
                            gerac-cod-ident-propost.num-propost         = propost.nr-proposta.
                   END.
              ASSIGN gerac-cod-ident-propost.cdn-ident-empres           = cd-empresa-aux
              ...
              then assign car-ide.dt-validade = b-import-bnfciar.dat-livre-1.
                     ...
                     then do:
                            ...
                            then do:               
                                   ...
                                   THEN ds-mensagem-valdoc = "nulo".
                                   RUN pi-cria-tt-erros(ds-mensagem-valdoc).
                                   leave l-inclusao. 
                                 end.
                          end.
...
procedure trata-afericao:
    ...
    then do:
           ...
           then  assign propost.nr-pessoas-titulares   = propost.nr-pessoas-titulares + 1.
           else  assign propost.nr-pessoas-dependentes = propost.nr-pessoas-dependentes + 1.                                        
         end.
...
PROCEDURE cria-modulos-benef:
    ...
    RUN escrever-log("@@@@@CRIA-MODULOS-BENEF P3. b-import-bnfciar.num-seqcial-bnfciar: " + STRING(b-import-bnfciar.num-seqcial-bnfciar)).
...
procedure cria-pessoa-fisica:
    ...
    assign tmpDemographic.id-pessoa                = 0
           ...
           tmpDemographic.nm-cartao                = b-import-bnfciar.nom-usuar
