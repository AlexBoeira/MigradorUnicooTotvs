/* CG0310X1.P - VALIDA CRIACAO DAS PROPOSTAS E TERMOS DE ADESAO*/

/** Campos reserva em uso:
 * IMPORT-PROPOST
 *    LOG-LIVRE-1:  utiliza beneficiario eventual?
 *    LOG-LIVRE-2:  tpnumeracao_familia (se eh normal ou automatico)
 *    LOG-LIVRE-3:  empresario individual? (novo campo RN432)
 *    LOG-LIVRE-5:  indicador se cobra franquia
 *    COD-LIVRE-2:  numero da lotacao ou 0. o codigo 1 indica DEMITIDO/APOSENTADO (fixo)
 *    COD-LIVRE-8:  hash MD5 das tabelas do UNICOO lidas para migrar a proposta para o TOTVS. usado para controlar atualizacoes
 *    DAT-LIVRE-1:  data de validade da carteira (CONTRATO_DA_PESSOA.DTVALIDCAR)
 *    
 *    NUM-LIVRE-2: contrato_da_pessoa.nrregistro
 *    NUM-LIVRE-3: contrato_da_pessoa.nrcontrato
 *    COD-LIVRE-3: contrato_da_pessoa.cdcontrato_padrao
 
 *    NUM-LIVRE-4: numero da familia ou 0
 *    NUM-LIVRE-5: id pessoa do contratante origem (pessoa que paga no unicoo)
 *    NUM-LIVRE-8: in-tipo-regulamentacao 1-REG; 2-NREG; 3-ADAPTADO
 *    NUM-LIVRE-10: nr-proposta. eh gerado no PL/SQL
 *
 * IMPORT-LOTAC-PROPOST
 *    NUM-LIVRE-1
 *    NUM-LIVRE-2
 *    NUM-LIVRE-3: NRDIA_DE_VENCIMENTO
 *    DAT-LIVRE-1: DATA INICIO LOTACAO NA PROPOSTA - informativo
 *    DAT-LIVRE-2: DATA FIM LOTACAO NA PROPOSTA - informativo
 *    LOG-LIVRE-1: UTILIZA NOME DA LOTACAO NO DOCUMENTO DE IDENTIFICACAO?
 *
 * LOTAC-PROPOST
 *    LOG-LIVRE-2: lg-fatura-lotac (indica se fatura por lotacao. se SIM, fatura contra o RESPONSAVEL FINANCEIRO.
 *                                                                se NAO, fatura normalmente para o contratante do termo de adesao) 
 *
 * PROPOST
 *    NUM-LIVRE-16: responsavel financeiro(0-desabilitado, 1-opcional, 2-obrigatorio)
 *    CHAR-11     : AA-MM-PROX-REAJUSTE (agrupadores de participacao)
 *    DATE-1      : data de validade da carteira (CONTRATO_DA_PESSOA.DTVALIDCAR)
 *    LOG-13      : inadimplencia a nivel de proposta
 *    DEC-1       : numero maximo de dias de inadimplencia
 *    LOG-4       : utiliza reajuste da regra de mensalidade?
 *    CHAR-15     : empresario individual? (novo campo RN432). valor: 'yes/no'
 *
 * TER-ADE
 *    LOG-2       : Faturamento Proporcional?
 *    LOG-5       : Cobrar Reaj Retroativo Parcelado?
 *
 ...
 * 14/02/2017 - implementado tratamento de IMPORT-LOTAC-PROPOST, para popular LOTAC-PROPOST com o novo conceito de LOTACAO.
 *              apenas planos com modalidade JURIDICA terao lotacao.
 *              os planos JURIDICOS que nao possuem lotacao no UNICOO terao lotacao 0
 *              a lotacao 1 eh reservada para DEMITIDOS/APOSENTADOS
 */


/* ultimo ID de mensagem de erro: 177 */
...
then do:
       ...
       run escrever-log("Parametros gerais do sistema nao cadastrados.").
       return.
     end.
...
then do:
       run escrever-log("Parametros dos modulos VP e MC nao localizados.").
       return.
     end.
...
ELSE do:
     ...
     run escrever-log("@@@@@saindo de cg0310x1.p").
...
procedure importa-dados:
    ...
    run escrever-log("@@@@@antes do loop principal").
    /**
...
*/
    run escrever-log("@@@@@depois da validacao da parametrizacao dos modulos de seguro").
    ...
    /*REPEAT:*/
        ...
        /* retirado pois afeta muito a performance (5 minutos)
        IF NOT CAN-FIND (FIRST import-propost where import-propost.ind-sit-import = in-status-monitorar-par) 
        THEN do:
               run escrever-log("@@@@@NADA NA FILA " + in-status-monitorar-par + " PARA IMPORTAR. SAINDO DO PROCESSO.").
               LEAVE.
        end.*/

        run escrever-log("@@@@@antes do for each principal (com exclusive-lock...)").
        ...
        for each import-propost fields (cd-modalidade 
           ...
           where import-propost.ind-sit-import = in-status-monitorar-par exclusive-lock query-tuning(no-index-hint):
            ...
            run escrever-log(in-status-monitorar-par + " Imp.Proposta Reg: " + string(import-propost.num-livre-2) + " Contr:" + string(import-propost.num-livre-3)).
            ...
            FOR FIRST modalid fields(cd-modalidade
                                     lg-aceita-repasse
                                     lg-repasse-obrig
                                     cd-tipo-medicina
                                     in-tipo-pessoa
                                     lg-usa-faixa-etaria) NO-LOCK
                WHERE modalid.cd-modalidade = import-propost.cd-modalidade:
            END.
            ...
            /* --- CONSISTE LOTACAO PROPOSTA ------ */
            assign lg-relat-erro = no.
            RUN consiste-dados-lotacao.
        ...
        run escrever-log("@@@@@depois do for each principal (com exclusive-lock...)").
    ...
    run escrever-log("@@@@@depois do loop principal").
...
procedure consiste-dados-proposta:
    ...
    THEN DO:
           ...
           then do:
                  ...
                  run pi-cria-tt-erros (9,"Tipo de Plano deve obrigar responsavel em prop. de seguro assist." +
                                        " Modalidade: " + STRING(ti-pl-sa.cd-modalidade) +
                                        " Plano: " + STRING(ti-pl-sa.cd-plano) +
                                        " Tipo Plano: " + STRING(ti-pl-sa.cd-tipo-plano)).
                end.
    ...
    then do:
           assign lg-relat-erro = yes.
           run pi-cria-tt-erros (16,"Nao existe a Forma de pagamento (HPR0110T) para essa estrutura:" + 
           ...
           then do:
                  assign lg-relat-erro = yes.
                  run pi-cria-tt-erros (17,"Tipo/Dia de vencimento invalido p/ Proposta: " + STRING(import-propost.cdn-tip-vencto) + "/" + string(import-propost.num-dia-vencto)).
                end.
         end.
    ...
    then do:
           assign lg-relat-erro = yes.
           run pi-cria-tt-erros (20,"Nao Existe Faixa de Beneficiarios para este Convenio (HPR0210B)." +
    ...
    then do:
           assign lg-relat-erro = yes.
           IF import-propost.dat-propost <> ?
           THEN char-aux1 = STRING(import-propost.dat-propost).
           ELSE char-aux1 = "nulo".
           IF dt-inicio-comerc-aux <> ?
           THEN char-aux2 = string(dt-inicio-comerc-aux).
           ELSE char-aux2 = "nulo".
           IF dt-fim-comerc-aux <> ?
           THEN char-aux3 = string(dt-fim-comerc-aux).
           ELSE char-aux3 = "nulo".
           run pi-cria-tt-erros (27,"Periodo de Comercializacao do Plano Invalido. (HPR0110C)" +
                                 " Modalidade: " + string(import-propost.cd-modalidade) + 
                                 " Plano: " + STRING(import-propost.cd-plano) +
                                 " Tipo: " + STRING(import-propost.cd-tipo-plano) +
                                 " Data Proposta: " + char-aux1 +
                                 " Inicio Comercializacao: " + char-aux2 + 
                                 " Fim Comercializacao: " + char-aux3). 
         end.
    ...
    then do:
           assign lg-relat-erro = yes.
           run pi-cria-tt-erros (57,"Indicador de faixa etaria especial invalido"). 
         end.
    ...
    FOR EACH import-modul-propost FIELDS (cd-modulo cd-forma-pagto 
       ...
       WHERE import-modul-propost.num-seqcial-propost = import-propost.num-seqcial-propost NO-LOCK QUERY-TUNING (NO-INDEX-HINT):
        ...
        then do:
               ...
               run pi-cria-tt-erros (80,"Tabela de Preco dos modulos (HPR0110L) nao cadastrada." +
...
/* ------------------------------------------------------------------------------------------- */
PROCEDURE consiste-dados-lotacao:

    IF NOT paravpmc.log-livre-26 /* indicador se utiliza o novo conceito de LOTACAO */
    THEN DO:
           IF CAN-FIND(FIRST import-lotac-propost
                       WHERE import-lotac-propost.num-seqcial-propost = import-propost.num-seqcial-propost)
           THEN DO:
                  RUN pi-cria-tt-erros (173,"Sistema parametrizado para nao usar lotacao, e foi encontrado IMPORT-LOTAC-PROPOST para essa proposta. Mod: " 
                                        + STRING(import-propost.cd-modalidade)
                                        + " Prop: " + string(import-propost.num-livre-10)).
                  ASSIGN lg-relat-erro = YES.
                  RETURN.
                END.
         END.
    ELSE DO:
           IF modalid.in-tipo-pessoa = "F" /* LOTACAO somente se aplica para modalidades PESSOA JURIDICA */
           then do:
                  IF CAN-FIND(FIRST import-lotac-propost WHERE import-lotac-propost.num-seqcial-propost = import-propost.num-seqcial-propost)
                  THEN DO:
                         RUN pi-cria-tt-erros (174,"Lotacao somente pode ser utilizada para Modalidades do tipo Pessoa Juridica. Mod: " 
                                               + STRING(import-propost.cd-modalidade)
                                               + " Prop: " + string(import-propost.num-livre-10)).
                         ASSIGN lg-relat-erro = YES.
                         RETURN.
                ...
                END.
           ELSE DO:
                  /* validacao retirada pois o processo foi alterado para tornar a lotacao Opcional (solicitado pela Unimed Nova Iguacu)
                  IF NOT CAN-FIND(FIRST import-lotac-propost
                                  WHERE import-lotac-propost.num-seqcial-propost = import-propost.num-seqcial-propost)
                  THEN DO:
                         RUN pi-cria-tt-erros (175,"Sistema parametrizado para usar lotacao, e nao foi encontrado IMPORT-LOTAC-PROPOST para essa proposta. Mod: " 
                                               + STRING(import-propost.cd-modalidade)
                                               + " Prop: " + string(import-propost.num-livre-10)).
                         ASSIGN lg-relat-erro = YES.
                         RETURN.
    ...
    FOR EACH import-lotac-propost FIELDS (cdn-lotac
                                          cdn-respons-financ)
       WHERE import-lotac-propost.num-seqcial-propost = import-propost.num-seqcial-propost NO-LOCK QUERY-TUNING (NO-INDEX-HINT):

        IF import-lotac-propost.cdn-lotac <> 0
        AND NOT CAN-FIND(FIRST lotac WHERE lotac.cdn-lotac = import-lotac-propost.cdn-lotac)
        then do:
               RUN pi-cria-tt-erros (176,"Lotacao cod: " + STRING(import-lotac-propost.cdn-lotac) 
                                     + " nao cadastrada para ser associada a proposta Mod: " 
                                     + STRING(import-propost.cd-modalidade)
                                     + " Prop: " + string(import-propost.num-livre-10)).
               assign lg-relat-erro = YES.
               NEXT.
        ...
        IF import-lotac-propost.cdn-respons-financ <> 0
        AND import-lotac-propost.cdn-respons-financ <> ?
        AND NOT CAN-FIND(FIRST contrat WHERE contrat.cd-contratante = import-lotac-propost.cdn-respons-financ)
        then do:
               IF import-lotac-propost.cdn-respons-financ <> ?
               THEN char-aux1 = STRING(import-lotac-propost.cdn-respons-financ).
               ELSE char-aux1 = "nulo".

               IF import-lotac-propost.cdn-lotac <> ?
               THEN char-aux2 = STRING(import-lotac-propost.cdn-lotac).
               ELSE char-aux2 = "nulo".
               
               IF import-propost.cd-modalidade <> ?
               THEN char-aux3 = STRING(import-propost.cd-modalidade).
               ELSE char-aux3 = "nulo".
               
               IF import-propost.num-livre-10 <> ?
               THEN char-aux4 = string(import-propost.num-livre-10).
               ELSE char-aux4 = "nulo".
               
               RUN pi-cria-tt-erros (177,"Contratante : " + char-aux1 
                                     + " nao encontrado para ser associado a Lotacao: " + char-aux2
                                     + " Mod: " + char-aux3
                                     + " Prop: " + char-aux4).
               assign lg-relat-erro = YES.
               NEXT.
...
PROCEDURE consiste-dados-faixa:
    ...
    FOR EACH import-faixa-propost FIELDS (cd-grau-parentesco nr-faixa-etaria num-idade-min num-idade-max qtd-fator-multiplic qtd-fator-multiplic-inscr)
       where import-faixa-propost.num-seqcial-propost = import-propost.num-seqcial-propost NO-LOCK QUERY-TUNING (NO-INDEX-HINT):
        ...
        then do:
               run pi-cria-tt-erros2 (104,"Idade Minima maior que Idade Maxima da Faixa Etaria Especial",
                                      "Grau Parentesco: " + string(import-faixa-propost.cd-grau-parentesco) +
                                      ". Faixa: " + string(import-faixa-propost.nr-faixa-etaria) + 
                                      ". Idade minima: " + string(import-faixa-propost.num-idade-min) +
                                      ". Idade maxima: " + string(import-faixa-propost.num-idade-max)).
               assign lg-relat-erro = yes.
             end.
    END.
